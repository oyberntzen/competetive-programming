...
vector<queue<long> > starting_items;
...
int main() {
    ...
    while (getline(cin, line)) {
        ...
        int i = 18;
        int last = 16;
        starting_items.push_back(queue<long>());
        for (; i < line.size(); i++) {
            if (line[i] == ',') {
        ...
        multiplied *= test_num[test_num.size()-1];
    ...
    for (int round = 0; round < 10000; round++) {
        for (int i = 0; i < starting_items.size(); i++) {
            while (starting_items[i].size() > 0) {
                long item = starting_items[i].front();
                starting_items[i].pop();

                long factor = operation_nums[i];
