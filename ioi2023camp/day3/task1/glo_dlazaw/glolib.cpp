...
int ruch(int x) {
    _initialize();
    _taken[x] = 1;
    std::cerr << "The contestant chooses employee number " << x << std::endl;
    for (int i = 2; i <= _n; i++) {
        if (_taken[i] == 0 && !_is_manager[i]) {
            _taken[i] = -1;
            std::cerr << "The jury selects employee number " << i << std::endl;
            return i;
        }
    }
    for (int i = _n; i > 1; i--) {
    ...
    if (_taken[1] > 0) {
        std::cout << "The contestant won" << std::endl;
    } else {
        std::cout << "The jury won" << std::endl;
    }
    exit(0);
}
